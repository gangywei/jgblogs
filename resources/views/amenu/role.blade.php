            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-header">
                        <form role="form" action="{{url('role')}}/{{$edit or 'search'}}" method="post">
                            {{ method_field('PUT') }}
                            {{ csrf_field() }}
                            @if(!isset($edit))
                                <a href="{{url('role\u\edit')}}"><button type="button" class="btn pull-right btn-info">回收站</button></a>
                            @endif
                            <div class="input-group col-xs-3 input-group">
                                <button type="button" data-toggle="modal" data-target="#file" class="btn pull-left btn-info">新增角色</button>
                            </div>
                        </form>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        <a href="{{url('role')}}" id="hidsrc" style="display: none"></a>
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            @foreach ($files as $file)
                                <tr>
                                    ...
                                    <td  data-id="{{$file->id}}">
                                        <a class="btn btn-primary btn-sm {{ isset($edit) ? '' : 'rev_file' }}" {{ isset($edit) ? "href =". url('role/create')."?id=$file->id": "data-toggle=modal data-target=#file"}} >{{ isset($edit) ? '还原' : '修改' }}</a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="box-primary">
                        <form role="form" id="model_from" action="{{url('role')}}" method="post">
                            {{ csrf_field() }}
                            <div class="box-body">
                                <div class="form-group">
                                    <label for="exampleInputPassword1">角色内容</label>
